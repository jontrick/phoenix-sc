...
has("var APP_VERSION='4.9.284'", 'version is 4.9.284');
...
(() => {
  ...
  // checks, rather than a number I remembered.
  {
    ...
    const KNOWN_ARMED = ['pepOpenEditStack', 'pepOpenCustomVial', 'pepOpenBloodPanel', 'pepOpenImport', 'pepOpenMakeUp', 'pepOpenSavePhase', 'pepOpenAddDose'];
    ...
    } else if (fns.length < 9 || absent.length) {
      bad(`PEP: STRUCTURAL the overlay enumeration itself broke — found ${fns.length} pepOpen* sheets (expected >= 9)${absent.length ? `, missing ${absent.join(', ')}` : ''}. Every keyboard guard is derived from this scan, so a silent miss here makes all of them pass vacuously.`);
    } else {
      miss.length === 0
...
// v4.9.263 — the 'overview' key now carries the label SCHEDULE. Jon asked for
// that screen to be the calendar and nothing else, with everything describing
// the protocol moved into ADJUST under "Protocol info". The KEY is unchanged so
// his persisted tab and every restore path still resolve.
[['today','TODAY'],['overview','SCHEDULE'],['stock','STOCK'],['adjust','ADJUST'],['order','ORDER'],['bloods','BLOODS']].forEach(([k,lab]) => {
  const t = lab;
