...
- **KNOWN RUNNER HAZARD (PM, unfixed as of 2026-08-21):** `functional_check.mjs` starts every case immediately and awaits them together, so **async cases interleave inside one shared sandbox** — a case can read another's localStorage mid-flight, and it presents as a bug in the code under test. Until it is serialised, keep async cases self-contained. Serialisation is blocked because suite-body-level state setup (rather than per-case) currently only works BECAUSE of the interleaving; any suite doing that must move setup per-case first.
