...
has("var APP_VERSION='4.9.259'", 'version is 4.9.259');
...
(() => {
  ...
  kb === 6
    ? ok('PEP: STRUCTURAL _phxKeyboardSafe armed at exactly 6 creation sites')
    : bad(`PEP: STRUCTURAL _phxKeyboardSafe armed ${kb} times — expected 6 (pepOpenEditStack, pepOpenCustomVial, pepOpenBloodPanel, pepOpenImport, pepOpenMakeUp, pepOpenSavePhase); the helper is NOT idempotent`);
  ...
  // checks, rather than a number I remembered.
  {
    ...
    const KNOWN_ARMED = ['pepOpenEditStack', 'pepOpenCustomVial', 'pepOpenBloodPanel', 'pepOpenImport', 'pepOpenMakeUp', 'pepOpenSavePhase'];
    ...
    } else if (fns.length < 8 || absent.length) {
      bad(`PEP: STRUCTURAL the overlay enumeration itself broke — found ${fns.length} pepOpen* sheets (expected >= 8)${absent.length ? `, missing ${absent.join(', ')}` : ''}. Every keyboard guard is derived from this scan, so a silent miss here makes all of them pass vacuously.`);
    } else {
      miss.length === 0
